...
func moveCursor(s *TerminalRenderer, newbuf *RenderBuffer, x, y int, overwrite bool) (seq string) {
	...
	// Fullscreen rendering must never use LF or RI as a cheap vertical
	// movement. At the physical bottom/top margin those bytes scroll the
	// terminal, so a one-row logical/physical cursor disagreement (for
	// example after an autowrap width mismatch) moves the complete frame and
	// makes later diffs appear as duplicate screens. CUP is margin-safe and
	// also re-synchronizes both coordinates on every cross-row movement.
	if s.flags.Contains(tFullscreen) && fy != y {
		return ansi.CursorPosition(x+1, y+1)
	}
