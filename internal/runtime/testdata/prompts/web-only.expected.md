...
| Keyword, topic, discovery, or current-information search | `WebSearch` |
| Fetch a known complete URL as static HTML, text, JSON, or another response | `WebFetch` |
| Render a known complete URL only when `WebFetch` is insufficient because the page requires JavaScript | `WebBrowse` |
...
For web work, do not send a known URL to `WebSearch`, and do not guess a URL
...
capability was unavailable or failed and why the fallback was necessary.
