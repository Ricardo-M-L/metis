...
func TestCheckResumeSize_OverCapErrors(t *testing.T) {
	...
	// Force the override to a tiny cap so we don't have to write 8 MiB.
	t.Setenv("METIS_RESUME_MAX_MB", "1") // 1 MiB
	id := "big-session"
...
func TestCheckResumeSize_DisabledByZeroEnv(t *testing.T) {
	...
	t.Setenv("METIS_RESUME_MAX_MB", "0")
	id := "huge"
...
func TestResumeMaxBytes_DefaultIsEightMiB(t *testing.T) {
