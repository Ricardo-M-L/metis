...
`v0.4.50` is a **formal CLI-only release** with GitHub `prerelease=false`, not a
...
- Registry channel `cli-only-stable` requires `prerelease=false` (the v0.4.50
...
- Draft lookup uses the complete authenticated release list, resolves an exact
  unique tag to its numeric ID, then reads by that ID. The tag REST endpoint
  excludes drafts. Only confirmed absence (lookup exit 4) permits creation;
  network/auth/JSON failures stop the workflow. `gh release view` must resolve
  the same ID before its draft-aware upload/download commands run.
...
1. Confirm the exact new tag is absent and the stable latest tag is recorded:
   ...
   ```sh
   git ls-remote --tags origin refs/tags/v0.4.50
   gh api repos/Ricardo-M-L/metis/releases/latest --jq .tag_name
   scripts/verify-release-policy.sh
   scripts/verify-dist.sh --metadata-only --tag v0.4.50
...
2. After separately reviewing/committing all intended CLI fixes, publish the
   reviewed source and ordinary `v0.4.50` tag using the repository's owner process.
...
3. Wait for the matching source/tag run to finish successfully, inspect its gates
   ...
   ```sh
   gh workflow run release-cli-publish.yml --ref main -f tag=v0.4.50 \
   ...
   the registered prerelease flag (`false` for v0.4.50) and `make_latest=false`, checks latest remained unchanged,
...
4. Explicitly dispatch the read-only published check and inspect its final result:
   ...
   ```sh
   gh workflow run release-published.yml --ref main -f tag=v0.4.50
   gh run list --workflow release-published.yml --limit 5
   gh api repos/Ricardo-M-L/metis/releases/tags/v0.4.50 \
...
```sh
METIS_VERSION=v0.4.50 METIS_INSTALL_DIR=/absolute/new/evaluation/bin \
